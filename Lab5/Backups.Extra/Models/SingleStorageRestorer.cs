...
public class SingleStorageRestorer : IRestorer
{
    public SingleStorageRestorer(IRepository repositoryFrom, IRepository repositoryTo)
    {
    ...
    public void RestoreToOriginalLocation(RestorePoint restorePoint)
    {
        ...
        List<IRepositoryObject> repositoryObjects =
            RepositoryFrom.GetDirectoryFiles(restorePointPath, new SingleStorageAlgorithm());

        string archiveName = repositoryObjects.First().GetName();
